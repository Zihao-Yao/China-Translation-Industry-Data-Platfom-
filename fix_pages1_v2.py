...
if idx > 0 and end_idx > 0:
    old_body3 = content[idx:end_idx+2]
    ...
    new_body3 = '\n'.join(new_body3_lines)
    
    content = content.replace(old_body3, new_body3)
    print('body3 replaced')
else:
    print('Could not find body3 region')

# Update script3
s3_old = "const script3 = sh.js_renderCos() + '\\n';"
s3_new = "const script3 = sh.js_renderCos() + '\\n';"
content = content.replace("const script3 = sh.js_renderCos() + '\\n';\nrenderCos()", s3_new)
content = content.replace("const script3 = sh.js_renderCos() + '\\n';\nrenderCos();", s3_new)
print('script3 updated (if found)')
